...
const LinkedInAnalysisDetail: React.FC<LinkedInAnalysisDetailProps> = ({ analysisId, onBack }) => {
  ...
  const scores = analysis.disc?.scores || { dominance: 0, influence: 0, steadiness: 0, compliance: 0 };
        ...
        <div className="bg-slate-900 rounded-[40px] p-10 text-white space-y-8 shadow-2xl">
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col items-end">
              ...
              <span className="text-lg font-bold text-blue-400">{getDiscLabel(analysis.disc?.predominant || 'N/A')}</span>
            </div>
          </div>

          <div className="grid grid-cols-4 gap-4 h-48 items-end border-b border-slate-800 pb-6">
            <div className="flex flex-col items-center gap-2">
              <div className="w-full bg-red-500 rounded-t-2xl transition-all shadow-lg shadow-red-500/20" style={{ height: `${scores.dominance}%` }}></div>
              <div className="flex flex-col items-center">
                <span className="text-xs font-black text-red-400">D</span>
                <span className="text-[10px] font-bold text-red-500/80">{scores.dominance}</span>
              </div>
            </div>
            <div className="flex flex-col items-center gap-2">
              <div className="w-full bg-amber-500 rounded-t-2xl transition-all shadow-lg shadow-amber-500/20" style={{ height: `${scores.influence}%` }}></div>
              <div className="flex flex-col items-center">
                <span className="text-xs font-black text-amber-400">I</span>
                <span className="text-[10px] font-bold text-amber-500/80">{scores.influence}</span>
              </div>
            </div>
            <div className="flex flex-col items-center gap-2">
              <div className="w-full bg-green-500 rounded-t-2xl transition-all shadow-lg shadow-green-500/20" style={{ height: `${scores.steadiness}%` }}></div>
              <div className="flex flex-col items-center">
                <span className="text-xs font-black text-green-400">S</span>
                <span className="text-[10px] font-bold text-green-500/80">{scores.steadiness}</span>
              </div>
            </div>
            <div className="flex flex-col items-center gap-2">
              <div className="w-full bg-blue-500 rounded-t-2xl transition-all shadow-lg shadow-blue-500/20" style={{ height: `${scores.compliance}%` }}></div>
              <div className="flex flex-col items-center">
                <span className="text-xs font-black text-blue-400">C</span>
                <span className="text-[10px] font-bold text-blue-500/80">{scores.compliance}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {[
              { l: 'D', t: 'Dominância', c: 'text-red-400 border-red-900/50 bg-red-950/20' },
              { l: 'I', t: 'Influência', c: 'text-amber-400 border-amber-900/50 bg-amber-950/20' },
              { l: 'S', t: 'Estabilidade', c: 'text-green-400 border-green-900/50 bg-green-950/20' },
              { l: 'C', t: 'Conformidade', c: 'text-blue-400 border-blue-900/50 bg-blue-950/20' },
            ].map(item => (
              <div key={item.l} className={`p-2 rounded-xl border ${item.c} flex flex-col items-center text-center`}>
                <span className="text-[8px] font-black uppercase tracking-tighter opacity-60 mb-0.5">{item.t}</span>
                <span className="text-[10px] font-bold">Pilar {item.l}</span>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-6">
            <div className="p-6 bg-slate-800/50 rounded-3xl border border-slate-700/50 space-y-2">
              ...
              <p className="text-sm text-slate-300 leading-relaxed italic">"{analysis.disc?.professionalDescription || 'Sem descrição disponível.'}"</p>
            </div>
            
            <div className="grid grid-cols-1 gap-4 text-xs">
              <div className="flex gap-4"><User className="text-slate-500 shrink-0" size={16} /> <span className="text-slate-400 font-bold uppercase tracking-tighter w-24">Foto:</span> <span className="text-slate-200">{analysis.disc?.photoAnalysis || 'N/A'}</span></div>
              <div className="flex gap-4"><FileText className="text-slate-500 shrink-0" size={16} /> <span className="text-slate-400 font-bold uppercase tracking-tighter w-24">Resumo:</span> <span className="text-slate-200">{analysis.disc?.summaryAnalysis || 'N/A'}</span></div>
              <div className="flex gap-4"><CheckCircle2 className="text-slate-500 shrink-0" size={16} /> <span className="text-slate-400 font-bold uppercase tracking-tighter w-24">Postagens:</span> <span className="text-slate-200">{analysis.disc?.postsAnalysis || 'N/A'}</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-sm space-y-8 flex flex-col">
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="p-6 bg-blue-50/50 rounded-[32px] border border-blue-100 space-y-3">
              ...
              <p className="text-sm text-blue-900 font-medium leading-relaxed italic">"{analysis.executiveSummary || 'Sem resumo disponível.'}"</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                )) || <span className="text-xs text-slate-400 italic">Nenhuma mapeada</span>}
              </div>
            </div>
          </div>
